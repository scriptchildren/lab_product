...
def get_word_freq(Q,K):
    fd_1 = nltk.FreqDist(preprocess(Q))
    fd_2 = nltk.FreqDist(preprocess(K))
    fd_1.plot(50, cumulative=False)
    fd_2.plot(50, cumulative=False)
    fig_Q = plt.gcf().canvas.draw()
    fig_K = plt.gcf().canvas.draw()
    return fig_Q, fig_K

def plot_words_freq(Q,K):
    fig_Q, fig_K = get_word_freq(Q,K)
    buf = io.BytesIO()
    fig_Q.savefig(buf, format='png')
    fig_K.savefig(buf, format='png')
...
def calc_ngram(Q,K):
    ...
    while True:
        ...
        print('一致した単語　　　　：',equal_token)
        print('一致した単語の出現頻度：',top10_freq)
...
def show_ngram():
    ...
    df1 = pd.DataFrame({'Number of matched words': calc_ngram(Q, K),
                        'Checked words': calc_ngram(Q, K),
                        'Similality': calc_ngram(Q, K),
                        'Checked words': calc_ngram(Q, K),
                        'Freaquecy of checked words': calc_ngram(Q, K)},
                        index=index)
    fig_Q, fig_K = plot_words_freq(Q,K)
    return render_template('table.html', title="ngram", name="show_name", df1=df1.to_html(), img_Q = fig_Q, img_K = fig_K) #render_template())
